...
def upgrade():
    ...
    for col in ("source","external_id","category","provider","network","status"):
    ...
    for col in ("kind","worker","is_error","created_at"):
    ...
    for col in ("category","created_at"):
        op.create_index(f"ix_ledger_{col}", "ledger", [col])
    ...
    for col in ("source","category","created_at"):
        op.create_index(f"ix_market_metrics_{col}", "market_metrics", [col])
